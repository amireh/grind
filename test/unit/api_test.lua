...
do
  ...
  cli:add_opt("-h, --host=HOST", "IP of the host grind is running on", "host", "127.0.0.1")
  cli:add_opt("-p, --port=PORT", "the port grind is accepting watchers on", "port", 11142)
  cli:add_opt("-t, --timeout=MS", "amount of milliseconds to wait before interrupting the connection", "tt", 1)
  cli:add_flag("-h", "displays the result without any extra syntax data in a human readable format", "ro", false)
