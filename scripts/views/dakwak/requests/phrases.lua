...
grind.define_view("dakwak", "requests", "phrases", 
  ...
  function(fmt, ctx, entry)
    ...
    if ctx.phrase and not ctx.checked_for_skipping then
      local b,e,skipped = extractors.skipped:find(entry.body)
      if b ~= nil then
        if skipped then
          ctx.digest = skipped
          ctx.status = "Skipped"
    ...
    elseif not ctx.trans then
      local b,e,id = extractors.trans:find(entry.body)
      if b ~= nil then
        ctx.trans = id
      end
    elseif not ctx.translated then
      local b,e,translated_or_not = extractors.translated:find(entry.body)
      if b ~= nil then
        ctx.status = translated_or_not == "was" and "Translated" or "Proxied"
        ctx.translated = true
      end
    elseif not ctx.linked then
      local b,e,linked_or_not = extractors.linked:find(entry.body)
      if b ~= nil then
