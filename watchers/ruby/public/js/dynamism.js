...
function locate_target(el) {
  ...
  if (!target_name || !target_index) {
    var parent = $(el).parents("[data-dyn-entity]:first");
    if (!parent)
      return false;

    target_name = parent.attr("data-dyn-entity");
    target_index = parent.attr("data-dyn-index");
  }
...
dyn_toggle = function() {
  var target = $(this).attr("data-dyn-target");
  if (target)
    $("#" + target).toggle();
  else
    $(this).toggle();
...
dyn_bind = function(parent) {
  ...
  parent.find("[data-dyn-target]:not([data-dyn-action]), [data-dyn-action='add']").click(dyn_add);
  parent.find("[data-dyn-action='remove']").click(dyn_rem);
  parent.find("[data-dyn-action='move_up']").click(dyn_move_up);
  parent.find("[data-dyn-action='move_down']").click(dyn_move_down);
  parent.find("[data-dyn-action='toggle']").click(dyn_toggle);
}
